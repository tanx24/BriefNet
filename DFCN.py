...
class Decoder(nn.Module):
    ...
    def forward(self, out1s, out2s, out3s, out4s, out5s):
        ...
        convs1 = self.convs1(out1s + F.interpolate(convs2, out1s.size()[2:], mode='bilinear', align_corners=True))
...
class ICM_ECM(nn.Module):
    def __init__(self, list_cvt, list_para):
        super(ICM_ECM, self).__init__()
    ...
    def forward(self, out1s, out2s, out3s, out4s, out5s, convs1, convs2, convs3, convs4, convs5):
        ...
        edge_feature, sal_feature, ecm_feature = [], [], []
        ...
        ecm_feature.append(msa1)
        ecm_feature.append(msa2)
        ecm_feature.append(msa3)
        ecm_feature.append(msa4)
        ecm_feature.append(msa5)

        return edge_feature, sal_feature, ecm_feature
...
class MergeLayer(nn.Module):
    ...
    def forward(self, edge_feature, sal_feature, ecm_feature):
        ...
        merge21 = torch.cat([merge11, ecm_feature[0]], dim=1)
        merge22 = torch.cat([merge12, ecm_feature[1]], dim=1)
        merge23 = torch.cat([merge13, ecm_feature[2]], dim=1)
        merge24 = torch.cat([merge14, ecm_feature[3]], dim=1)
        merge25 = torch.cat([merge15, ecm_feature[4]], dim=1)
        ...
        merge = merge21 + merge22 + sal13i + sal14i + sal15i
...
class DFCN(nn.Module):
    def __init__(self, cfg):
        ...
        self.iecm = ICM_ECM(list_cvt, list_para)
        ...
        self.outf2 = nn.Sequential(nn.Conv2d(list_cvt[2][1], list_cvt[1][1], kernel_size=list_para[0][1], padding=list_para[1][1]), nn.BatchNorm2d(list_cvt[1][1]), nn.ReLU(inplace=True), nn.Conv2d(list_cvt[1][1], 1, kernel_size=list_para[0][1], padding=list_para[1][1]))
        self.outf3 = nn.Sequential(nn.Conv2d(list_cvt[2][2], list_cvt[1][2], kernel_size=list_para[0][2], padding=list_para[1][2]), nn.BatchNorm2d(list_cvt[1][2]), nn.ReLU(inplace=True), nn.Conv2d(list_cvt[1][2], 1, kernel_size=list_para[0][2], padding=list_para[1][2]))
        self.outf4 = nn.Sequential(nn.Conv2d(list_cvt[2][3], list_cvt[1][3], kernel_size=list_para[0][3], padding=list_para[1][3]), nn.BatchNorm2d(list_cvt[1][3]), nn.ReLU(inplace=True), nn.Conv2d(list_cvt[1][3], 1, kernel_size=list_para[0][3], padding=list_para[1][3]))
        self.outf5 = nn.Sequential(nn.Conv2d(list_cvt[2][4], list_cvt[1][4], kernel_size=list_para[0][4], padding=list_para[1][4]), nn.BatchNorm2d(list_cvt[1][4]), nn.ReLU(inplace=True), nn.Conv2d(list_cvt[1][4], 1, kernel_size=list_para[0][4], padding=list_para[1][4]))
    ...
    def forward(self, x, shape=None):
        ...
        edge_feature, sal_feature, ecm_feature = self.iecm(out1s, out2s, out3s, out4s, out5s, convs1, convs2, convs3, convs4, convs5)
        ...
        up_sal_f.append(F.interpolate(self.outf2(merge_feature[1]), shape, mode='bilinear', align_corners=True))
        up_sal_f.append(F.interpolate(self.outf3(merge_feature[2]), shape, mode='bilinear', align_corners=True))
        up_sal_f.append(F.interpolate(self.outf4(merge_feature[3]), shape, mode='bilinear', align_corners=True))
        up_sal_f.append(F.interpolate(self.outf5(merge_feature[4]), shape, mode='bilinear', align_corners=True))
